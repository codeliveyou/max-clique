...
int POPULATION = 10; // size of the population
int LOCAL_IMPROVEMENT = 10; // number of local improvements
int GENERATIONS = 10000; // number of generations to run the algorithm
int MUTATIONS = 1; // How many vertices to remove randomly in the mutate() function
int UNIQUE_ITERATIONS = 100; // Used by localImprovement() to prevent a stall for very small cliques
int SHUFFLE_TOLERANCE = 10; // Generate a fresh population after a stall
...
struct DescendingSortedListNode {
   public: bool operator()(SortedListNode & node1, SortedListNode & node2) {
...
struct DescendingClique {
   public: bool operator()(Clique & c1, Clique & c2) {
...
inline void localImprovement(Clique & clique) {
   ...
   for (int i = 0; i < LOCAL_IMPROVEMENT; i++) {
      int rand1 = generateRandomNumber(0, (int)clique.clique.size() - 1);
      int rand2 = generateRandomNumber(0, (int)clique.clique.size() - 1);
      int countt = 0;
      while (rand1 == rand2) {
         countt++;
         ...
         rand1 = generateRandomNumber(0, (int)clique.clique.size() - 1);
         rand2 = generateRandomNumber(0, (int)clique.clique.size() - 1);
      }
...
inline void mutate(Clique & clique) {
   ...
   for (int i = 0; i < MUTATIONS; i++) {
      int rand = generateRandomNumber(0, (int)clique.clique.size() - 1);
      int count = 0;
      while (flags[rand]) {
         rand = generateRandomNumber(0, (int)clique.clique.size() - 1);
         count++;
   ...
   if (rand < 0.5) {
      ...
      while (clique.pa.size() > 0) {
         int rand = generateRandomNumber(0, (int)clique.pa.size() - 1);
...
std::vector < int > run_max_clique(const std::string & filename, int generations, int populationNum, int localImprovementNum, int mutations, int uniqueIterations, int shuffleTolerance) {
   srand(time(NULL));
   processData(filename.c_str());
   GENERATIONS = generations;
   POPULATION = populationNum;
   LOCAL_IMPROVEMENT = localImprovementNum;
   MUTATIONS = mutations;
   UNIQUE_ITERATIONS = uniqueIterations;
   SHUFFLE_TOLERANCE = shuffleTolerance;
