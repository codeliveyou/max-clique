...
class AntClique:
    ...
    def construct_clique(self, ant_idx):
        ...
        initial_vertex = random.sample(list(self.graph.keys()), 1)[0]
...
def ant_colony_algorithm(number_of_nodes: int, adjacency_list: list[list[int]]) -> list[int]:
    # Convert adjacency list to graph format expected by AntClique:
    # dict[node] -> dict[neighbor] -> attributes dict including pheromone
