...
def mcp(number_of_nodes: int, adjacency_list: list[list[int]]) -> list[int]:
    global MaxCliques
    global best_clique_nodes
    MaxCliques = 0
    best_clique_nodes = []
    ...
    #             nodeSet.add(ai)
    #             nodeSet.add(aj)
    for ai, adj in enumerate(adjacency_list):
        for aj in adj:
            if aj > ai:
                unedge = [ai, aj]
                edgeSet.add(tuple(unedge))
    ...
    if not PcoreList:
        ...
        best_clique_nodes.sort()
        print("Max Cliques1: ", len(best_clique_nodes), best_clique_nodes)
    else:
        ...
        best_clique_nodes.sort()
        print("Max Cliques: ", len(best_clique_nodes), best_clique_nodes)
